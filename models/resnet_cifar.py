...
class ResNet(nn.Module):
    def __init__(self, block, num_layers, covcfg, compress_rate, num_classes=10):
        ...
        self.layer1 = self._make_layer(block, 30, blocks=n, stride=1,
                                       compress_rate=compress_rate[1:2 * n + 1])
        self.layer2 = self._make_layer(block, 60, blocks=n, stride=2,
                                       compress_rate=compress_rate[2 * n + 1:4 * n + 1])
        self.layer3 = self._make_layer(block, 120, blocks=n, stride=2,
        ...
        if num_layers == 110:
            ...
            self.fc = nn.Linear(120 * block.expansion, num_classes)
